...
public class UserAuthJdbcHandler {
    ...
    public void exists(String email, Handler<AsyncResult<Boolean>> resultHandler) {
        connection.queryWithParams("SELECT COUNT(1) FROM eventbus.USERS WHERE EMAIL=?", new JsonArray().add(email), r -> {
            if (r.failed()) {
    ...
    public void insert(User user, String salt, String hash, Handler<AsyncResult<Long>> resultHandler) {
        connection.updateWithParams("INSERT INTO eventbus.USERS(ID,NAME,EMAIL,PASSWORD,PASSWORD_SALT) VALUES (?,?,?,?,?)",
                new JsonArray().add(user.getId()).add(user.getName()).add(user.getEmail()).add(hash).add(salt), r -> {
                    if (r.succeeded()) {
